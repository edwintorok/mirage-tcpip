(** Data structures that automatically update the global network stack memory usage in
    {!module:Mirage_net.Memory}
*)

module Lwt_dllist : sig
    ...
    *)
    ...
    (** [create size_of] creates a new empty sequence with [size_of] returning
        the amount of memory used in bytes by an element.
    *)
...
val update_lru: (module Lru.F.S with type t = 'a) -> old:'a -> 'a -> 'a
(** [update_lru (module Lru) ~old new] should be used for updating mutable LRU fields from [old] to [new].
    It updates the global network stack memory usage.
*)

val update_set: (module Set.S with type elt = 'a and type t = 'b) -> ('a -> int) -> old:'b -> 'b -> 'b
(** [update_set (module Set) size_of ~old new] should be used for updating mutable Set fields from [old] to [new].
    It updates the global network stack memory usage.
    [size_of] computes the size of one element.
*)
